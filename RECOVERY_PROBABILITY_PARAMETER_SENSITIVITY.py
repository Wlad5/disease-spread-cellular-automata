...
def setup_output_directories(initial_infected_count):
    ...
    csv_dir = f"recovery_probability_csv_infected_{initial_infected_count}"
    img_dir = f"recovery_probability_images_infected_{initial_infected_count}"
...
def save_results_to_csv(csv_dir, recovery_values, ca_results, ode_results):
    ...
    for i, recovery_val in enumerate(recovery_values):
        ...
        filename = f"recovery_prob_{recovery_val:.4f}.csv"
...
def recovery_prob_sensitivity_experiment():
    ...
    # Save results to CSV
    print(f"\nSaving CSV files...")
    save_results_to_csv(csv_dir, recovery_prob_values, ca_results, ode_results)
    
    # Update the plotting function to handle mean ± std
    plot_comparison_with_error("recovery_prob", recovery_prob_values, ca_results, ode_results, params['t_max'], params['ode_dt'], plots_per_figure=5, img_dir=img_dir)

    # Calculate norms and plot norm vs parameter
    norms = calculate_norm(ca_results, ode_results)
    plot_norm_vs_parameter("recovery_prob", recovery_prob_values, norms, img_dir=img_dir)
