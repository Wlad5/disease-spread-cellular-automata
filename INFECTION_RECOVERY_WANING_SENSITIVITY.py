"""
Sensitivity Analysis: Infection + Recovery (varied together), then Waning

In this analysis:
- infection_prob and recovery_prob are varied together
- waning_prob is varied for multiple values of the combined parameter
"""
...
infection_recovery_pairs = list(zip(infection_prob_range, recovery_prob_range))
# Waning varied for each pair
waning_prob_values = np.linspace(0.0, 1, 20)
...
# ==========================================================
def plot_comparison_with_error(ir_pair_idx, ir_pair, waning_values, ca_results, ode_results, 
                               t_max, ode_dt, plots_per_figure=3):
    """
    Plot comparison between CA and ODE for fixed infection+recovery pair,
    varying waning probability. Save plots to disk.
    """
    num_waning = len(waning_values)
    num_figures = int(np.ceil(num_waning / plots_per_figure))
    
    inf_prob, rec_prob = ir_pair
    
    for fig_idx in range(num_figures):
        fig = plt.figure(figsize=(16, 12))
        start_idx = fig_idx * plots_per_figure
        end_idx = min(start_idx + plots_per_figure, num_waning)
        batch_size = end_idx - start_idx
        
        for batch_pos, i in enumerate(range(start_idx, end_idx)):
            val = waning_values[i]
            mean_ca, std_ca = ca_results[ir_pair_idx][i]
            ode_time, mean_ode_states, std_ode_states = ode_results[ir_pair_idx][i]
            ...
            ax_s.set_title(f"S: waning={val:.4f}", fontsize=11)
            ...
            ax_i.set_title(f"I: waning={val:.4f}", fontsize=11)
            ...
            ax_r.set_title(f"R: waning={val:.4f}", fontsize=11)
        ...
        plt.suptitle(f"Infection+Recovery (inf={inf_prob:.3f}, rec={rec_prob:.3f}) vs Waning " +
                    f"(Batch {fig_idx + 1}/{num_figures})", fontsize=16, y=0.98)
        plt.subplots_adjust(hspace=0.35, wspace=0.15)
        
        # Save figure instead of showing
        filename = os.path.join(img_dir, f"ir_pair_{ir_pair_idx}_batch_{fig_idx}.png")
...
def save_results_to_csv(ir_pair_idx, ir_pair, waning_values, ca_results, ode_results):
    ...
    for i, waning_val in enumerate(waning_values):
        ...
        # CA results
        ca_filename = os.path.join(csv_dir, f"ca_ir_{ir_pair_idx}_waning_{i}.csv")
        ...
        # ODE results
        ode_filename = os.path.join(csv_dir, f"ode_ir_{ir_pair_idx}_waning_{i}.csv")
...
def save_norms_to_csv(waning_values, norms_by_pair, infection_recovery_pairs):
    """Save norm values to CSV"""
    norms_filename = os.path.join(csv_dir, "norms_summary.csv")
    with open(norms_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['ir_pair_idx', 'inf_prob', 'rec_prob', 'waning_val', 'norm_S', 'norm_I', 'norm_R'])
        
        for pair_idx, (inf_prob, rec_prob) in enumerate(infection_recovery_pairs):
            norms = norms_by_pair[pair_idx]
            for i, waning_val in enumerate(waning_values):
                writer.writerow([pair_idx, inf_prob, rec_prob, waning_val,
                               norms['S'][i], norms['I'][i], norms['R'][i]])

def plot_norm_vs_parameter(waning_values, norms_by_pair, infection_recovery_pairs):
    """Plot norms for each infection+recovery pair and save separately"""
    for pair_idx, (inf_prob, rec_prob) in enumerate(infection_recovery_pairs):
        fig = plt.figure(figsize=(10, 6))
        norms = norms_by_pair[pair_idx]
        plt.plot(waning_values, norms['S'], marker='o', linestyle='-', label='Norm (S)', color='b', linewidth=2)
        plt.plot(waning_values, norms['I'], marker='o', linestyle='-', label='Norm (I)', color='r', linewidth=2)
        plt.plot(waning_values, norms['R'], marker='o', linestyle='-', label='Norm (R)', color='g', linewidth=2)
        plt.xlabel('Waning Probability', fontsize=12)
        plt.ylabel('L2 Norm', fontsize=12)
        plt.title(f'Norms vs Waning (inf={inf_prob:.3f}, rec={rec_prob:.3f})', fontsize=14)
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        # Save figure
        filename = os.path.join(img_dir, f"norm_ir_pair_{pair_idx}.png")
        plt.savefig(filename, dpi=150, bbox_inches='tight')
        plt.close(fig)
...
def sensitivity_experiment():
    ...
    print(f"Running sensitivity for Infection+Recovery, then Waning...")
    all_ca_results = []
    all_ode_results = []

    for ir_idx, (inf_prob, rec_prob) in enumerate(infection_recovery_pairs):
        print(f"\n--- Infection={inf_prob:.3f}, Recovery={rec_prob:.3f} ---")
        ca_results = []
        ode_results = []

        for waning_val in waning_prob_values:
            p = params.copy()
            ...
            print(f"  waning={waning_val:.4f} → CA I₀={ca_initial_infected_frac:.5f}")
        ...
        # Save results to CSV
        save_results_to_csv(ir_idx, (inf_prob, rec_prob), waning_prob_values, all_ca_results, all_ode_results)
        
        # Plot for this pair
        plot_comparison_with_error(ir_idx, (inf_prob, rec_prob), waning_prob_values, 
    ...
    save_norms_to_csv(waning_prob_values, norms_by_pair, infection_recovery_pairs)
    plot_norm_vs_parameter(waning_prob_values, norms_by_pair, infection_recovery_pairs)
