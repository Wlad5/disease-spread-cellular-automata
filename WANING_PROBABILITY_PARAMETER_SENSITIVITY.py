...
def calculate_norm(ca_results, ode_results):
    ...
    for i in range(len(ca_results)):
        ...
        interp_ode_S = np.interp(ca_timesteps, ode_time, mean_ode_states[:, 0])
        interp_ode_I = np.interp(ca_timesteps, ode_time, mean_ode_states[:, 1])
        interp_ode_R = np.interp(ca_timesteps, ode_time, mean_ode_states[:, 2])

        # Calculate L2 norms for S, I, and R
        norm_S = np.sqrt(np.sum((mean_ca['S_frac'] - interp_ode_S) ** 2))
        norm_I = np.sqrt(np.sum((mean_ca['I_frac'] - interp_ode_I) ** 2))
        norm_R = np.sqrt(np.sum((mean_ca['R_frac'] - interp_ode_R) ** 2))
...
def waning_prob_sensitivity_experiment():
    ...
    plot_norm_vs_parameter("waning_prob", waning_prob_values, norms)
