"""
Sensitivity Analysis: Recovery + Waning (varied together), then Infection

In this analysis:
- recovery_prob and waning_prob are varied together
- infection_prob is varied for multiple values of the combined parameter
"""
...
plots_per_figure = 5  # Show 3 plots per figure
...
# ==========================================================
def plot_comparison_with_error(rw_pair_idx, rw_pair, infection_values, ca_results, ode_results, 
                               t_max, ode_dt, plots_per_figure=3):
    """
    Plot comparison between CA and ODE for fixed recovery+waning pair,
    varying infection probability. Save plots to disk.
    """
    num_infection = len(infection_values)
    num_figures = int(np.ceil(num_infection / plots_per_figure))
    
    rec_prob, wan_prob = rw_pair
    
    for fig_idx in range(num_figures):
        fig = plt.figure(figsize=(16, 12))
        start_idx = fig_idx * plots_per_figure
        end_idx = min(start_idx + plots_per_figure, num_infection)
        batch_size = end_idx - start_idx
        
        for batch_pos, i in enumerate(range(start_idx, end_idx)):
            val = infection_values[i]
            mean_ca, std_ca = ca_results[rw_pair_idx][i]
            ode_time, mean_ode_states, std_ode_states = ode_results[rw_pair_idx][i]
            ...
            ax_s.set_title(f"S: infection={val:.4f}", fontsize=11)
            ...
            ax_i.set_title(f"I: infection={val:.4f}", fontsize=11)
            ...
            ax_r.set_title(f"R: infection={val:.4f}", fontsize=11)
        ...
        plt.suptitle(f"Recovery+Waning (rec={rec_prob:.3f}, wan={wan_prob:.4f}) vs Infection " +
                    f"(Batch {fig_idx + 1}/{num_figures})", fontsize=16, y=0.98)
        plt.subplots_adjust(hspace=0.35, wspace=0.15)
        
        # Save figure instead of showing
        filename = os.path.join(img_dir, f"rw_pair_{rw_pair_idx}_batch_{fig_idx}.png")
...
def plot_norm_vs_parameter(infection_values, norms_by_pair, recovery_waning_pairs):
    """Plot norms for each recovery+waning pair and save separately"""
    for pair_idx, (rec_prob, wan_prob) in enumerate(recovery_waning_pairs):
        fig = plt.figure(figsize=(10, 6))
        norms = norms_by_pair[pair_idx]
        plt.plot(infection_values, norms['S'], marker='o', linestyle='-', label='Norm (S)', color='b', linewidth=2)
        plt.plot(infection_values, norms['I'], marker='o', linestyle='-', label='Norm (I)', color='r', linewidth=2)
        plt.plot(infection_values, norms['R'], marker='o', linestyle='-', label='Norm (R)', color='g', linewidth=2)
        plt.xlabel('Infection Probability', fontsize=12)
        plt.ylabel('L2 Norm', fontsize=12)
        plt.title(f'Norms vs Infection (rec={rec_prob:.3f}, wan={wan_prob:.4f})', fontsize=14)
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        # Save figure
        filename = os.path.join(img_dir, f"norm_rw_pair_{pair_idx}.png")
        plt.savefig(filename, dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  Saved norm plot: {filename}")

def save_results_to_csv(rw_pair_idx, rw_pair, infection_values, ca_results, ode_results):
    """Save CA and ODE results to CSV files"""
    rec_prob, wan_prob = rw_pair
    
    for i, infection_val in enumerate(infection_values):
        mean_ca, std_ca = ca_results[rw_pair_idx][i]
        ode_time, mean_ode_states, std_ode_states = ode_results[rw_pair_idx][i]
        
        # CA results
        ca_filename = os.path.join(csv_dir, f"ca_rw_{rw_pair_idx}_infection_{i}.csv")
        ...
        # ODE results
        ode_filename = os.path.join(csv_dir, f"ode_rw_{rw_pair_idx}_infection_{i}.csv")
...
def save_norms_to_csv(infection_values, norms_by_pair, recovery_waning_pairs):
    """Save norm values to CSV"""
    norms_filename = os.path.join(csv_dir, "norms_summary.csv")
    with open(norms_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['rw_pair_idx', 'rec_prob', 'wan_prob', 'infection_val', 'norm_S', 'norm_I', 'norm_R'])
        
        for pair_idx, (rec_prob, wan_prob) in enumerate(recovery_waning_pairs):
            norms = norms_by_pair[pair_idx]
            for i, infection_val in enumerate(infection_values):
                writer.writerow([pair_idx, rec_prob, wan_prob, infection_val,
...
def sensitivity_experiment():
    ...
    print(f"Running sensitivity for Recovery+Waning, then Infection...")
    all_ca_results = []
    all_ode_results = []

    for rw_idx, (rec_prob, wan_prob) in enumerate(recovery_waning_pairs):
        print(f"\n--- Recovery={rec_prob:.3f}, Waning={wan_prob:.4f} ---")
        ca_results = []
        ode_results = []

        for infection_val in infection_prob_values:
            p = params.copy()
            p['infection_prob'] = infection_val
            p['recovery_prob'] = rec_prob
            ...
            print(f"  infection={infection_val:.4f} → CA I₀={ca_initial_infected_frac:.5f}")
        ...
        # Save results to CSV for this pair
        for i, infection_val in enumerate(infection_prob_values):
            save_results_to_csv(rw_idx, (rec_prob, wan_prob), infection_prob_values, all_ca_results, all_ode_results)
        
        # Plot for this pair
        plot_comparison_with_error(rw_idx, (rec_prob, wan_prob), infection_prob_values, 
    ...
    plot_norm_vs_parameter(infection_prob_values, norms_by_pair, recovery_waning_pairs)
    save_norms_to_csv(infection_prob_values, norms_by_pair, recovery_waning_pairs)
    
    print(f"\n✓ Analysis complete!")
    print(f"  CSV outputs: {csv_dir}")
    print(f"  Image outputs: {img_dir}")
