"""
Recovery Probability + Waning Probability Combined Sensitivity Analysis

Analyzes how disease dynamics (S, I, R) vary with both recovery_prob and waning_prob.
...
def _plot_partial_heatmaps(norms_S, norms_I, norms_R, recovery_labels_partial, waning_labels, param_idx, total_params):
    ...
    # Plot heatmap for S
    sns.heatmap(data_S, xticklabels=waning_labels, yticklabels=recovery_labels_partial,
                cmap='YlOrRd', ax=axes[0], cbar_kws={'label': 'L2 Norm'},
    ...
    # Plot heatmap for I
    sns.heatmap(data_I, xticklabels=waning_labels, yticklabels=recovery_labels_partial,
                cmap='YlOrRd', ax=axes[1], cbar_kws={'label': 'L2 Norm'},
                annot=True, fmt='.3f', vmin=0, vmax=0.5)
    axes[1].set_title('L2 Norm: Infected (I)', fontsize=12, fontweight='bold')
    axes[1].set_xlabel('Waning Probability', fontsize=11)
    ...
    # Plot heatmap for R
    sns.heatmap(data_R, xticklabels=waning_labels, yticklabels=recovery_labels_partial,
                cmap='YlOrRd', ax=axes[2], cbar_kws={'label': 'L2 Norm'},
                annot=True, fmt='.3f', vmin=0, vmax=0.5)
    axes[2].set_title('L2 Norm: Recovered (R)', fontsize=12, fontweight='bold')
    axes[2].set_xlabel('Waning Probability', fontsize=11)
...
def recovery_waning_combined_sensitivity_experiment():
    ...
    # Store results: results[recovery_idx][waning_idx] = (ca_history, ode_time, ode_states)
    all_results = []
    ...
    for recovery_idx, recovery_prob in enumerate(recovery_prob_values):
        ...
        print(f"\n{'='*70}")
        print(f"Recovery Probability: {recovery_prob:.4f}")
        print(f"{'='*70}")
        ...
        # PLOT AFTER EACH RECOVERY PROBABILITY COMPLETES
        print(f"\nGenerating plots for recovery prob {recovery_prob:.4f}...")
...
if __name__ == "__main__":
    recovery_waning_combined_sensitivity_experiment()
